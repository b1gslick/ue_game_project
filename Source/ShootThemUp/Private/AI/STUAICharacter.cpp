...
void ASTUAICharacter::OnHealthChanged(float Health, float HealthDelta)
{
    ...
    const auto HealtBarWidget = Cast<USTUHealthBarWidget>(HealthWidgetComponent->GetUserWidgetObject());
    if (!HealtBarWidget) return;
    HealtBarWidget->SetHealthPercent(HealthComponent->GetHealthPercent());
}
